...
// ─── Directus Flow UUID — Security Checklist ─────────────────────────────────
// Các Flow UUIDs bên dưới được bundle vào client (APK/IPA). Đây là thiết kế bình thường
// vì client cần UUID để gọi API — tương tự như gọi REST endpoint.
//
// ⚠️  PHẢI đảm bảo phía Directus:
//   ✅ REGISTER_FLOW          → public (không cần auth) — cố ý
//   ✅ ONBOARDING_EMAIL_CHECK → public — cố ý
//   ✅ RESET_PASSWORD_FLOW    → public (chỉ chấp nhận reset_token hợp lệ) — cố ý
//   🔒 TẤT CẢ flows còn lại  → PHẢI yêu cầu Bearer token hợp lệ trong Directus
//      (Settings → Flows → chọn flow → Access Control → require authentication)
//
// Nếu một flow bị gọi không cần auth ngoài ý muốn → lỗ hổng leo thang đặc quyền.
// ─────────────────────────────────────────────────────────────────────────────

// API Configuration
export const API_URL = Constants.expoConfig?.extra?.API_URL || "https://marutek.space";
...
export const REGISTER_FLOW_ID =
  (Constants.expoConfig?.extra?.REGISTER_FLOW_ID as string | undefined) ??
...
export const ONBOARDING_EMAIL_CHECK_FLOW_ID =
  (Constants.expoConfig?.extra?.ONBOARDING_EMAIL_CHECK_FLOW_ID as string | undefined) ??
 ...
 * Override IDs via app.json extra.* when backend changes.
 */
export const EXERCISE_COUNT_BY_TOPIC_FLOW_ID =
  (Constants.expoConfig?.extra?.EXERCISE_COUNT_BY_TOPIC_FLOW_ID as string | undefined) ??
...
export const EXERCISE_COUNT_BY_LESSON_AND_TOPIC_FLOW_ID =
  (Constants.expoConfig?.extra?.EXERCISE_COUNT_BY_LESSON_AND_TOPIC_FLOW_ID as string | undefined) ??
 ...
 * Override via `app.json` -> `expo.extra.*` when switching backend.
 */
export const UPDATE_PROFILE_FLOW_ID =
  (Constants.expoConfig?.extra?.UPDATE_PROFILE_FLOW_ID as string | undefined) ??
...
export const UPDATE_TARGET_USER_FLOW_ID =
  (Constants.expoConfig?.extra?.UPDATE_TARGET_USER_FLOW_ID as string | undefined) ??
...
export const GET_TARGETS_FLOW_ID =
  (Constants.expoConfig?.extra?.GET_TARGETS_FLOW_ID as string | undefined) ??
...
export const ACCOUNT_TYPE_FLOW_ID =
  (Constants.expoConfig?.extra?.ACCOUNT_TYPE_FLOW_ID as string | undefined) ??
...
export const UPDATE_TOPICS_OF_INTEREST_FLOW_ID =
  (Constants.expoConfig?.extra?.UPDATE_TOPICS_OF_INTEREST_FLOW_ID as string | undefined) ??
...
export const VOCAB_DETAIL_FLOW_ID =
  (Constants.expoConfig?.extra?.VOCAB_DETAIL_FLOW_ID as string | undefined) ??
...
export const UPDATE_NOTE_VOCAB_FLOW_ID =
  (Constants.expoConfig?.extra?.UPDATE_NOTE_VOCAB_FLOW_ID as string | undefined) ??
...
export const ADD_NOTE_TO_VOCAB_FLOW_ID =
  (Constants.expoConfig?.extra?.ADD_NOTE_TO_VOCAB_FLOW_ID as string | undefined) ??
...
export const EXERCISE_BY_ID_FLOW_ID =
  (Constants.expoConfig?.extra?.EXERCISE_BY_ID_FLOW_ID as string | undefined) ??
...
export const SUBMIT_EXERCISE_FLOW_ID =
  (Constants.expoConfig?.extra?.SUBMIT_EXERCISE_FLOW_ID as string | undefined) ??
...
export const FLASHCARD_DECK_VOCABS_FLOW_ID =
  (Constants.expoConfig?.extra?.FLASHCARD_DECK_VOCABS_FLOW_ID as string | undefined) ??
...
export const UPDATE_HSK_LEVEL_FLOW_ID =
  (Constants.expoConfig?.extra?.UPDATE_HSK_LEVEL_FLOW_ID as string | undefined) ??
  "9cb381de-f1a6-4ef7-af80-18d370ff8733";
export const UPDATE_HSK_LEVEL_FLOW_PATH = `/flows/trigger/${UPDATE_HSK_LEVEL_FLOW_ID}`;

/**
 * Reset password — URL cố định giống Postman (không dựng từ template /uuid).
 * Override toàn bộ URL qua extra.RESET_PASSWORD_FLOW_URL nếu cần.
 */
export const RESET_PASSWORD_FLOW_URL =
  (Constants.expoConfig?.extra?.RESET_PASSWORD_FLOW_URL as string | undefined) ??
...
export const MARUTEK_CONFIG = {
  API_URL:
    process.env.EXPO_PUBLIC_MARUTEK_API_URL ||
    Constants.expoConfig?.extra?.MARUTEK_API_URL ||
...
/** YouTube Data API v3 (thumbnail, snippet). Set `EXPO_PUBLIC_YOUTUBE_API_KEY` in `.env`. */
