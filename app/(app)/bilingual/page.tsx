...
function BilingualListPageContent() {
    ...
    const levelQueryParam = selectedLevel === "Tất cả" ? undefined : selectedLevel.replace(" ", "");
        ...
        <PageContainer>
                            ...
                            >
                                ...
                                <div className="relative aspect-video w-full overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                                    ...
                                    <Badge className="absolute left-3 bottom-3 rounded-full bg-amber-500 text-white font-bold hover:bg-amber-600 border-none px-3 py-0.5 text-[10px] uppercase tracking-wider shadow-sm">
                                        {item.level ? item.level.replace("HSK", "HSK ") : "HSK"}
                                    </Badge>
            ...
            {!isLoading && !error && displayItems.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 text-center flex-1">
                    ...
                    <p className="mt-2 text-zinc-500 font-medium">Không tìm thấy bài đọc nào phù hợp với bộ lọc.</p>
                    {selectedLevel !== "Tất cả" && (
                        <Button
                            onClick={() => handleLevelChange("Tất cả")}
